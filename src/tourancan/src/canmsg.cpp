...
//////发送配置信息/////////////////////
void canmessages::sendRadarConfig() 
{
...
{   
    ...
    boost::function<void()> send = boost::bind(&canmessages::sendRadarConfig, this);
    boost::thread sendRadarConfigThread(send);  

  // receive thread for LRR
  boost::function<void()> recLrr = boost::bind(&canmessages::runLrrRadarMain, this);

  boost::thread runLrrRadarMainThread(recLrr); 
  flagaa = false;
    }
...
void canmessages::runLrrRadarMain()
{
  int ret;
  bool isReady;
  ...
  { 
	...
	//shuju jie bao
      isReady = recLrrRadarData(ret); //一次接收一个数据包，一个包包括很多帧数据，ret/13是帧数，处理接受 过来的数据 如果所有帧处理完毕，则isReady=True，则publish目标 
...
bool canmessages::recLrrRadarData(uint16 num_buf) 
{
  uint16 i, j;
  int num=0;
  int numall=0;
